...
module "network" {
  ...
  /*
    Use o meta-argumento `depends_on´ para lidar com dependências ocultas de recursos ou módulos que o Terraform não pode inferir automaticamente. 
    Você só precisa especificar explicitamente uma dependência quando um recurso ou módulo depende do comportamento de outro recurso, 
    mas não acessa nenhum dado desse recurso em seus argumentos.
  */
