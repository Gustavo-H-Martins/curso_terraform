...
# As referências de caminho local permitem fatorar partes de uma configuração em um único repositório de origem.
module "network" {
  # Um caminho local deve começar com ou ./para ../indicar que um caminho local se destina, para distinguir de um endereço de registro do módulo .
  source = "./network"
